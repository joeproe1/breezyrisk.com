...
Vendor concentration is one of five incident types where coverage breaks down across all three bank policies. I map the full picture [here]({{ '/insights/five-incidents-three-policies/' | relative_url }}). A [Risk Intelligence Report]({{ '/risk-intelligence-report/' | relative_url }}) models exactly this scenario: your core vendor offline for five days, run against your dependent BI sublimit, waiting period, and trigger language, so the dollar gap between the exposure and the coverage is on paper before the outage happens.
